...
import {
  ...
  vertexGlobal,
  vertexMain
} from './modules/Dissolve/shaders/index'
...
export function useDissolveEffect() {
  ...
  const initDissolveEffect = (dom: any) => {
    ...
    dissolve = new Dissolve(dom, (currentDom) => {
      resizeBloomComposer(currentDom)
    })
    particleUniforms = {
      uPixelDensity: {
    ...
    physicalMaterial.onBeforeCompile = (shader: any) => {
      // handle dissolve effect with edges
    ...
    animate()
    dissolve?.dom?.addEventListener('resize', onWindowResize)
  }
  ...
  const animate = () => {
    ...
    if (autoProgress) {
      ...
      if (
        dissolveUniformData.uProgress.value >= 14.0 ||
        dissolveUniformData.uProgress.value <= -14.0
      )
        direction *= -1
  ...
  const destroy = () => {
    try {
      ...
      dissolve?.renderer && (dissolve.renderer.content = null)
