...
export function useNightEarth() {
  ...
  let glowMaterial: any, glow: any, earthMaterial: any, composer: any, atmosphere: any,
    bloomPass: any;
  // 加载动画
  ...
  const animate = () => {
    ...
    if (camera && glow && glowMaterial) {
      glowMaterial.uniforms.viewVector.value = new THREE.Vector3().subVectors(camera.position, glow.position);
    }
    // 添加闪烁效果
    if (Math.random() > 0.95) {
